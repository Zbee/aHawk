    ...
    if realms[realm][item][0] == True:
        for sub in subscriptions:
            ...
            if method != 3:
                ...
                if checkId == check:
                    link = int(vals[3])
                    itemName = open("/var/www/ahawk/assets/data/items/" + str(
                        item) + ".dat", "r")
                    itemName = itemName.readlines()[0]
                    if method == 1:
                        email = emails[link][0]
                        msg = MIMEText(
                            "Hello,\nCurrently there are " + str(
                                realms[realm][item][2])
                            + " " + itemName + " available on " + realm +
                            ".\nYou can get "
                            + "more information about this item here: "
                              "https://ahawk.zbee.me"
                            + "/items?item=" + str(
                                item) + "&realm=" + realm + "\n\nYou "
                            + "have received this email because you, "
                              "or someone pretending "
                            + "to be you, signed this email address up for "
                              "notifications "
                            + "on aHawk.\nIf this was not you, then you can "
                              "unsubscribe "
                            + "using this link: "
                              "https://ahawk.zbee.me/sub/unsubscribe.php?"
                            + "token=" + emails[link][
                        ...
                        s.sendmail("noreply@ahawk.zbee.me", [email],
                                   msg.as_string())
                        s.quit()
                    elif method == 2:
                        data = {
                            ...
                            "value3":
                                "https://us.battle.net/wow/en/vault/character/auction"
                        ...
                        response = urllib.request.urlopen(req, simplejson.dumps(
                            data))
