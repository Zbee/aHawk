...
$url = explode('lowestPricePer/', $url)[1];
$url = explode('/', $url);
...
$availability = $controller->availabilityOf($item, $realm, $time, 'lowestPricePer');
...
$echo = [
  'lowestPricePer' => $time == 'today'
    ? $availability->return
    : $availability->lowestPricePer
];

if (!is_array($echo['lowestPricePer'])) $echo['time'] = $availability->time;
